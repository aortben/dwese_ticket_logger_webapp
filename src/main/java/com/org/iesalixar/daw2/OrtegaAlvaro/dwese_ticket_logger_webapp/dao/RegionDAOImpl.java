...
public class RegionDAOImpl implements RegionDAO {
    ...
    public void deleteRegion(Long id) {
        ...
        try {
            ...
            String deleteCommunitiesSql = "DELETE FROM comunidades WHERE region_id = ?";
            int deletedCommunities = jdbcTemplate.update(deleteCommunitiesSql, id);
            logger.info("Deleted {} comunidades related to region {}", deletedCommunities, id);

            String deleteRegionSql = "DELETE FROM regions WHERE id = ?";
            int rowsAffected = jdbcTemplate.update(deleteRegionSql, id);
            logger.info("Deleted region. Rows affected: {}", rowsAffected);

        } catch (Exception e) {
            logger.error("Error deleting region with id {}: {}", id, e.getMessage());
            throw e;
        }
    }
    /**
    ...
    public Region getRegionById(Long id) {
        ...
        try {
            Region region = jdbcTemplate.queryForObject(sql, new BeanPropertyRowMapper<>(Region.class), id);logger.info("Region retrieved: {} - {}", region.getCode(), region.getName());
            return region;
        } catch (Exception e) {
